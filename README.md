...
# -final_presentation-
# 개발 진척도
스테이트 - 메인 스테이트 60% 외에 제대로 개발하지 못함.
적 - 적 종류를 추가하지 못함. 기존 계획 종류 5가지에 1가지밖에 추가하지 못함. 적 탄환이 발사하긴 하지만 소리와 인공지능을 구현하지 못함. - 50%
플레이어 - 탄환이 발사되고 충돌체크가 되며 탄환 발사시 소리도 남. 라이프 구현을 하지 못했고 적 탄환과의 충돌체크는 하지 못했다. 플레이어의 탄환과 적 충돌체크는 구현함. - 80%
애니메이션 - start 애니메이션과 죽는 애니메이션을 구현하지 못함. - 50%
게임 진행도 - 끝이 정해져 있지 않아 게임의 진행도가 미흡함. - 20%
# 주차별 커밋 횟수
![이미지][/https://user-images.githubusercontent.com/34496211/101283111-ea3beb80-381b-11eb-80c7-31af4e055e53.PNG]
# 기말 프로젝트를 하며 겪은 어려움
내가 무엇을 개발하고 싶었는지, 개발에 대한 생각이 어떻게 잡혀 있는지 잘 모르겠다. 무언가를 개발하는 것에 크게 흥미를 느끼는 나를 찾지 못했다. 내가 무엇을 개발하고 싶은지 구체화하는 작업을 건너뛴 것 같아 개발을 진행하는데 어려움이 있었다. 무엇을 개발하려는 건지 뚜렷한 목표를 가지지 않으니 잡고 있어도 뭘 하는지 잘 모르는 상태였다. 진전없이 시간낭비만 한 듯 하다.
