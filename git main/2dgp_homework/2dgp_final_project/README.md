...
# 현재까지 진행 상황
|캐릭터 컨트롤|상하좌우 이동O, space bar누르면 탄막 발사(캐릭터가 1개이고 방향키로 움직이며 위치상 space bar가 더 적절해보여 변경) - 100%|
|충돌체크|적 탄막과 메인 캐릭터 충돌체크X, 메인캐릭터 탄막과 적 충돌체크O - 50%|
|애니메이션|시작등장X, 폭파X, 탄막이동O, 개체이동O - 50%|
|진행|적 패턴 구현이 아직 미숙함, 전체적인 게임의 흐름이 잡히지 않음 - 10%|
# main game state object
|class 종류| 내용 |
|bullet|player의 bullet만 적용된다. enemy bullet은 따로 만들어야함. 플레이어가 spacebar를 누르면 탄막이 발사된다.|
|enemy|생성 패턴이 덜 구현됨, 현재는 일정시간마다 생성. 체력도 구현해야함. 죽는 애니메이션 추가 예정|
|player(plane)|이동, 탄막 구현 완료. 탄막의 업그레이드와 죽는 애니메이션을 구현할 예정|
|background|세로 스크롤되는 background구현|
|ui|score 구현|
|sound추가 예정 Boss는 미정.|
